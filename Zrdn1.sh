...
echo "$time,Zrdn1,функционирует" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "messages/alive/$time.log"
...
# if [ -d "$directory" ]; then
    ...
    while true; do
            ...
            if [ "$circle" -eq 1 ]; then
                ...
                if ! grep -q "^$id " "$file2_path"; then
                    if grep -q "^$id " "$file3_path"; then
                        ...
                        echo "$time,Zrdn1,цель не уничтожена,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                        ...
                        if ! grep -q "^($id " "$file3_path"; then
                            if grep -q "^$id " "$file_path"; then
                                ...
                                if [[ -n $speed_info && $speed_info != 0 ]]; then
                                    ...
                                    echo "$time,Zrdn1,цель подтверждена,$id,$x $y,$speed_info,$type" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                                    if [[ $rocket -gt 0 && ($type == "крылатая ракета" || $type == "самолет") ]]; then
                                        ...
                                        echo "$time,Zrdn1,выстрел,$id,$x $y,$speed_info,$type" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                                        echo "$time,Zrdn1,осталось снарядов,$rocket" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/-$time.log"
                                    ...
                                    if [[ $rocket == 0 ]]; then
                                        ...
                                        echo "$time,Zrdn1,снаряды закочились" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                                        rocket=$(( rocket - 1 ))
                                    fi
                                    speed_info=0
                                fi
        ...
        while IFS= read -r line; do
            ...
            echo "$time,Zrdn1,уничтожена цель,$line" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"

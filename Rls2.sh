...
echo "$time,Rls2,функционирует" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "messages/alive/$time.log"
            ...
            if ! grep -q "^$id " "$file2_path"; then
               ...
               if grep -q "^$id " "$file_path"; then
                  ...
                  if [[ -n $speed_info && $speed_info != 0 ]]; then
                     ...
                     # echo "$time,Rls2,цель подтверждена,$id,$x $y,$speed_info,$type" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                     ...
                     if [[ $to_spro == "true" && $type == "баллистическая ракета" && $(echo "${distance1} > ${distance2}" | bc -l) ]]; then
                        ...
                        echo "$time, Подтверждена цель с id: $id, X: $x Y: $y скорость: $speed_info тип: $type"
                        echo "$time,Rls2,цель подтверждена,$id,$x $y,$speed_info,$type, движется к СПРО" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log"
                     else
                        time=$(TZ=Europe/Moscow date +"$time_format")
